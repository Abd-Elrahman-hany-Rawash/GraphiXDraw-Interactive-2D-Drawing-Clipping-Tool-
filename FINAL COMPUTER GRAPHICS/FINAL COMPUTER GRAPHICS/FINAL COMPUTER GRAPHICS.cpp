...
bool HDCToFile(const char* FilePath, HDC Context, RECT Area, uint16_t BitsPerPixel)
{
    ...
    BITMAPINFO Info;             //structure defines the dimensions and color information for a DIB.
    BITMAPFILEHEADER Header;        //structure that contains information about the dimensions of color format
    memset(&Info, 0, sizeof(Info));
    memset(&Header, 0, sizeof(Header));
    Info.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);  //bsize :Specifies the number of bytes required by the structure. This value does not include the size of the color table or the size of the color masks, if they are appended to the end of structure. See Remarks.
    ...
    int nCmdShow)
{
    ...
    wincl.hCursor = LoadCursor(NULL, IDC_CROSS);/*----------------------MOUCE-B-------------*/
    wincl.lpszMenuName = NULL;                 /* No menu */
    wincl.cbClsExtra = 0;                      /* No extra bytes after the window class */
    wincl.cbWndExtra = 0;                      /* structure or the window instance */
    /* Use Windows's default colour as the background of the window */
    wincl.hbrBackground = (HBRUSH)RGB(255,255,0);/*-----------BackGround-A-------*/
